...
class IssueService {
  async createIssue(issueData) {
    ...
    if (type === 'campaign_withdrawal' && relatedEntity.type !== 'Campaign') {
      throw new Error('For campaign_withdrawal, relatedEntity must be Campaign');
    }
    ...
    if (type === 'campaign_withdrawal') {
      ...
      if (!campaign.volunteers.some(v => v.user.toString() === issueData.reportedBy.toString())) {
      ...
      if (!task.assignedUsers.some(u => u.userId.toString() === issueData.reportedBy.toString())) {
    ...
    if (type === 'campaign_withdrawal') {
      ...
      const campaign = await Campaign.findById(relatedEntity.entityId).populate('createdBy');
    ...
    if (relatedEntity.type === 'Campaign') {
      await Campaign.findByIdAndUpdate(relatedEntity.entityId, { $push: { issues: newIssue._id } });
    } else if (relatedEntity.type === 'Task') {
      await Task.findByIdAndUpdate(relatedEntity.entityId, { $push: { issues: newIssue._id } });
    }

    return newIssue.populate('reportedBy assignedTo');
  }

  async getIssues(filters, user) { // user giờ là { _id }
    const query = {};
    if (filters.type) query.type = filters.type;
    if (filters.status) query.status = filters.status;
    // Loại bỏ check role, giả sử middleware đã filter quyền xem (nếu cần chỉ thấy issues của mình, có thể add query.reportedBy = user._id luôn)
    // query.reportedBy = user._id; // Uncomment nếu muốn users chỉ thấy issues của mình mà không cần role

    return await Issue.find(query).populate('reportedBy assignedTo');
  }

  async getIssueById(id) {
    return await Issue.findById(id).populate('reportedBy assignedTo');
  }

  async updateIssue(id, updateData, user) { // user là { _id }
    const issue = await Issue.findById(id);
    if (!issue) throw new Error('Issue not found');

    // Loại bỏ check role, chỉ check nếu assignedTo
    ...
    // Specific logic on status change
    if (updateData.status === 'resolved' || updateData.status === 'closed') {
      if (issue.type === 'campaign_withdrawal') {
        ...
        campaign.volunteers = campaign.volunteers.filter(v => v.user.toString() !== issue.reportedBy.toString());
        await campaign.save();

        // Update user joinedCampaigns
        await User.findByIdAndUpdate(issue.reportedBy, { $pull: { joinedCampaigns: issue.relatedEntity.entityId } });
        ...
        // Ví dụ: await Task.findByIdAndUpdate(issue.relatedEntity.entityId, { 'review.status': 'resolved' });
      }
    }

    await issue.save();
    return issue.populate('reportedBy assignedTo');
  }

  async deleteIssue(id, user) { // user là { _id }
    const issue = await Issue.findById(id);
    if (!issue) throw new Error('Issue not found');
    ...
    if (issue.relatedEntity.type === 'Campaign') {
      await Campaign.findByIdAndUpdate(issue.relatedEntity.entityId, { $pull: { issues: id } });
    } else if (issue.relatedEntity.type === 'Task') {
      await Task.findByIdAndUpdate(issue.relatedEntity.entityId, { $pull: { issues: id } });
    }
